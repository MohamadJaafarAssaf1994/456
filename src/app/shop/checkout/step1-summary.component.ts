    ...
    <div *ngIf="items$ | async as items; else empty">
      ...
      <div class="mt-6 border-t pt-4">
        ...
        <div class="flex gap-2">
          ...
          />
          <button (click)="applyPromo()" class="px-4 py-2 bg-gray-800 text-white rounded">
            Appliquer
          </button>
        </div>
      ...
      <div class="mt-6 text-lg">
        ...
        <ng-template #normalTotal>
          <div class="text-xl font-bold">Total: {{ total$ | async }} €</div>
        </ng-template>
      </div>
...
export class Step1SummaryComponent {
  ...
  applyPromo(): void {
    ...
    this.store.dispatch(CheckoutActions.applyPromo({ code: this.promoCode.trim() }));
  }
}
